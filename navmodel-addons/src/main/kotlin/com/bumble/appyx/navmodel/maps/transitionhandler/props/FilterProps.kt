...
internal fun Modifier.filtersModifier(
    ...
    val hiddenBeforeShown = Props(alpha = 0f, offsetY = 0.dp)
    val hiddenAfterShown = Props(alpha = 0f, offsetY = -(80).dp)

    fun stateToProps(state: Maps.State): Props =
        when (state) {
            Maps.State.CREATED -> hiddenBeforeShown
            Maps.State.DEFAULT -> hiddenBeforeShown
            Maps.State.VENUES_LIST -> shown
            Maps.State.VENUES_PAGER -> shown
            Maps.State.VENUE_SHOW -> hiddenAfterShown
            Maps.State.DESTROYED -> hiddenAfterShown
        }
